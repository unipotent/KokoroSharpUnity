...
public static class Segmentation {
    /// <summary> Turns the input tokens into multiple segments, aggressively optimized for streaming. </summary>
    ...
    public static List<int[]> SplitToSegments(int[] tokens, int minFirstSegmentLength = 1, int maxFirstSegmentLength = 60, int maxSecondSegmentLength = 100, int minFollowupSegmentsLength = 150) {
        ...
        if (tokens.Length < maxSecondSegmentLength + potentialSegments[0].end + 1) { potentialSegments.Add((potentialSegments[0].end + 1, tokens.Length)); }
            ...
            if (potentialSegments.Count == 1) {
                ...
                if (potentialSegments.Count == 1) { potentialSegments.Add((potentialSegments[0].end + 1, potentialSegments[0].end + maxFirstSegmentLength)); }
            }
        }
